...
import type { AiFoodData } from "@/ai/flows/search-foods-flow";
...
export default function DailyTrackerPage() {
  ...
  const dailyTotals = useMemo(() => {
    return loggedFoods.reduce(
      (acc, logged) => {
        const ratio = logged.quantity / 100; // Nutrients are per 100g
        acc.calories += logged.foodData.calories * ratio;
        acc.protein += logged.foodData.protein * ratio;
        acc.carbs += logged.foodData.carbs * ratio;
        acc.fat += logged.foodData.fat * ratio;
        return acc;
      },
  ...
  const handleAddFood = (foodData: AiFoodData, quantity: number, mealType: MealType) => {
    const newLog: LoggedFood = {
      logId: Date.now().toString(),
      mealType,
      quantity,
      foodData: {
        id: foodData.name.toLowerCase().replace(/ /g, '-'),
        name: foodData.name,
    ...
    toast({
      title: "Food Added!",
      description: `${foodData.name} added to ${mealType}.`,
    });
  };
          ...
          <Accordion type="multiple" defaultValue={mealOrder} className="space-y-3">
            {mealOrder.map(mealType => {
                ...
                const totalCalories = loggedItems.reduce((acc, log) => {
                    ...
                    return acc + (log.foodData.calories * ratio);
                }, 0);
...
const LoggedFoodItem: FC<{
  ...
  onDelete: (logId: string) => void;
    ...
    const { foodData, quantity } = loggedFood;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors group">
            <div className="flex items-center gap-3 flex-1">
                <div className="flex-grow min-w-0">
                    <p className="font-semibold truncate">{foodData.name}</p>
                    <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                        <span>{quantity}g</span>
                        <span>•</span>
                        <span className="flex items-center gap-1">
                            <Beef className="h-3 w-3 text-red-500" />
                            {calculatedNutrients.protein.toFixed(0)}g
                        </span>
                        <span>•</span>
                        <span className="flex items-center gap-1">
                            <Wheat className="h-3 w-3 text-yellow-600" />
                            {calculatedNutrients.carbs.toFixed(0)}g
                        </span>
                        <span>•</span>
                        <span className="flex items-center gap-1">
                            <Droplets className="h-3 w-3 text-blue-500" />
                            {calculatedNutrients.fat.toFixed(0)}g
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div className="flex items-center justify-between sm:justify-end gap-4">
                <p className="font-medium text-sm sm:text-base">{Math.round(calculatedNutrients.calories)} kcal</p>
                ...
                <div className="hidden sm:flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <AlertDialog>
                        ...
                        <AlertDialogContent className="w-[95vw] max-w-md rounded-lg">
                            <AlertDialogHeader>
                                <AlertDialogTitle>Remove {foodData.name}?</AlertDialogTitle>
                                <AlertDialogDescription>
