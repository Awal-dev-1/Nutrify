...
--- Instructions ---
1.  **Filter Foods**: From the \`availableFoods\` list provided below, first filter out any foods that do not match the user's \`dietaryPreferences\`. Check the 'tags' array for each food. For example, if the user is 'Vegan', only consider foods with a 'Vegan' tag. If the preference is 'None', skip this filtering step.
...
3.  **Score Filtered Foods**: Score the remaining foods based on the user's \`primaryGoal\`.
    *   **If goal is 'lose-weight'**: Prioritize lower-calorie, higher-protein foods.
        *   Base Score: 50. Add 30 points if calories < 400. Add 20 points if protein > 20. Add 10 points if fat < 15.
    *   **If goal is 'gain-weight'**: Prioritize higher-calorie, higher-protein/carb foods.
        *   Base Score: 50. Add 30 points if calories > 500. Add 20 points if protein > 25.
    *   **If goal is 'maintain-weight' or 'eat-healthier'**: Prioritize balanced, nutrient-dense foods.
        *   Give a base score of 70 to all items that passed the filter.
4.  **Generate Reason**: For each of the top foods, create a short, encouraging \`reason\` explaining why it's a good choice for their goal.
...
5.  **Sort and Select**: Sort all scored foods by their final score in descending order.
6.  **Add Randomness**: From the top 10 sorted foods, shuffle them slightly to ensure variety on each request.
7.  **Final Output**: Return the top 5 foods from the shuffled list. If fewer than 5 foods are available after filtering, return all of them. Ensure the output strictly adheres to the JSON schema. Use the original 'id' from the food item as the 'foodId' in the output. Map the food properties (name, calories, protein, carbs, fat) directly.
